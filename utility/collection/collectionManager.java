...
/**
 * @author Zvavamwe Tafadzwa Zvavamwe
 * initialing our commands
 */
public class collectionManager {
    private Deque <Flat> flats ;

    public collectionManager(Deque<Flat> flats, File xmlCollection) {
        this.flats = flats;
    ...
    private Deque<String> commandsinfo ;
    {
        ...
        // Making a manual
        commandsinfo = new ArrayDeque<>();
        commandsinfo.offer("help - display help for available commands");
        commandsinfo.offer("info print all elements in string representation to standard output");
        commandsinfo.offer("add add new element to the collection");
        commandsinfo.offer("update_by_id id - update the element`s value, whose ID is equal to the given." +
                " You should to enter ID after entering a command.");
        commandsinfo.offer("remove_by_id id - remove an element from the collection by its ID." +
                " You should to enter ID after entering a command.");
        commandsinfo.offer("clear - clear the collection");
        commandsinfo.offer("save - save the collection to file");
        commandsinfo.offer("execute_script filename - read and execute a script from specified file." +
                " You should to enter path to file after entering a command.");
        commandsinfo.offer("exit - end the program (without saving to file)");
        ...
        commandsinfo.offer("sum_of_height - print the sum of the values of the height field for all elements" +
                " of the collection");
        commandsinfo.offer("group_counting_by_nationality - group collection items by field value " +
                "nationality, display the number of items in each group");
        commandsinfo.offer("count_greater_than_nationality nationality - print the number of elements, value" +
        ...
        try {
            for ( ; ; ) {
               // System.getproperty("java.classpath") ;
                System.out.print("Enter a full path to XML file with collection boi: ");
                        ...
                        while ((e = xmlEventReader.peek()) != null) {
                            ...
                            if (e.isStartElement() && ((StartElement) e).getName().equals(qName)) {
                                ...
                                if (unmarshalledFlat.getId() != 0 && !unmarshalledFlat.getName().equals(null) &&
                                        ...
                                        && !unmarshalledFlat.equals(null) && !unmarshalledFlat.getLivingSpace().equals(null)
                                        && !unmarshalledFlat.getFurnish().equals(null) && !newCoordinates.getX().equals(null) &&
                                        !newCoordinates.getY().equals(null) && !unmarshalledFlat.getName().equals(null)) {
    ...
    public void help() {
        //for (Map.Entry<String, String> entry : commandsinfo.stream()) {
     ...
     */
    public Integer receiveXLocation() {
        for ( ; ; ) {
            try {
    ...
    public House receiveHOUSE() {
        return new House(receiveName(), receiveYLocation());
    }
     ...
     */
    public Furnisher receiveFurnish() {
        for ( ; ; ) {
            try {
                ...
                switch (furnisherChoose) {
                    case 1:
                        return Furnisher.NONE;
                    case 2:
                        return Furnisher.FINE;
                    case 3:
                        return Furnisher.BAD;
                    default:
                        break;
                }
    ...
    public void add() {
        Flat newFlat = new Flat(receiveId(), receiveName(), receiveCoordinates(), returnDate(),
                receiveArea(), receiveFurnish(), receiveView(),ReceiveNumberOfRooms(),receiveLivingSpace(),receiveHOUSE());
        flats.add(newFlat);
    }
    ...
    public void update_by_id(String id) {
        for (Flat flat : flats) {
            ...
            if (strId.equals(id)) {
                flats.remove(flat);
                Flat updatedFlat = new Flat(Math.toIntExact(intId), receiveName(), receiveCoordinates(), flat.returnCreationDate(),
                                        receiveArea(), receiveFurnish(), receiveView(), ReceiveNumberOfRooms(), receiveLivingSpace(), receiveHOUSE());
            ...
            while((command = reader.readLine()) != null) {
                ...
                switch (finalUserCommand[0]) {
                    ...
                    case "exit":
                        exit();
                    case "add_if_min":
                        add_if_min(new Flat(receiveId(), receiveName(), receiveCoordinates(), returnDate(),
                                receiveArea(), receiveFurnish(), receiveView()
                                ));
                        break;
                    case "remove_greater":
